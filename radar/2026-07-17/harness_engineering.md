...
- **Feedback quality**: agents are only as good as the signals the harness feeds back - fast tests, linters, typed errors and structured tool outputs beat raw logs. See [Evaluation Driven AI Development](/architecture-pattern/evaluation_driven_development/) for measuring agent workflows.
